import { User, Pet, Post, ActivityFeedItem, FriendRequest, Playdate, Message, Role, UserStatus, PrivacySettings, PetPrivacySettings, HealthLogEntry, PetAchievement, FavoriteItem, Visibility } from '../types';
...
const allInitialUsers: Omit<User, 'pets'|'friends'|'incomingFriendRequests'|'outgoingFriendRequests'|'likedFeedItems'>[] = [
    { id: 'user-1', name: 'Alice Johnson', username: 'alicej', avatarUrl: 'https://i.pravatar.cc/150?u=user-1', role: 'admin', status: 'active', privacySettings: { profileBasics: 'public', pets: 'public', activity: 'public', friends: 'public' } },
    { id: 'user-2', name: 'Bob Williams', username: 'bobw', avatarUrl: 'https://i.pravatar.cc/150?u=user-2', role: 'moderator', status: 'active', privacySettings: { profileBasics: 'public', pets: 'friends', activity: 'public', friends: 'friends' } },
    { id: 'user-3', name: 'Charlie Brown', username: 'charlieb', avatarUrl: 'https://i.pravatar.cc/150?u=user-3', role: 'user', status: 'active', privacySettings: { profileBasics: 'friends', pets: 'friends', activity: 'private', friends: 'private' } },
    { id: 'user-4', name: 'Diana Miller', username: 'dianam', avatarUrl: 'https://i.pravatar.cc/150?u=user-4', role: 'user', status: 'active', privacySettings: { profileBasics: 'public', pets: 'public', activity: 'public', friends: 'friends' } },
    { id: 'user-5', name: 'Ethan Davis', username: 'ethand', avatarUrl: 'https://i.pravatar.cc/150?u=user-5', role: 'user', status: 'suspended', privacySettings: { profileBasics: 'public', pets: 'public', activity: 'public', friends: 'public' } },
];

const allInitialPets = [
    { id: 'pet-1', ownerId: 'user-1', name: 'Buddy', type: 'Dog', breed: 'Golden Retriever', avatarUrl: 'https://placedog.net/500/500?id=1', birthdate: '2018-05-12', bio: 'Loves fetch and long walks at the beach. A bit of a goofball but has a heart of gold. His favorite toy is a squeaky hedgehog.', photos: ['https://placedog.net/600/400?id=1', 'https://placedog.net/600/400?id=2'], friends: ['pet-2', 'pet-5'], privacySettings: { profile: 'public', playdates: 'friends' } },
    { id: 'pet-2', ownerId: 'user-2', name: 'Lucy', type: 'Cat', breed: 'Siamese', avatarUrl: 'https://placekitten.com/500/500?image=1', birthdate: '2020-09-20', bio: 'A very vocal and curious cat. Enjoys sunbathing and judging everyone from a distance. Will do anything for a treat.', photos: ['https://placekitten.com/600/400?image=2'], friends: ['pet-1'], privacySettings: { profile: 'friends', playdates: 'friends' } },
    ...
    { id: 'pet-5', ownerId: 'user-4', name: 'Rocky', type: 'Dog', breed: 'Boxer', avatarUrl: 'https://placedog.net/500/500?id=5', birthdate: '2019-03-15', bio: 'Full of energy! Loves to play tug-of-war.', photos: [], friends: ['pet-1'], privacySettings: { profile: 'public', playdates: 'friends' } },
    ...
    { id: 'pet-7', ownerId: 'user-1', name: 'Gizmo', type: 'Dog', breed: 'Pug', avatarUrl: 'https://placedog.net/500/500?id=7', birthdate: '2022-01-10', bio: 'A bundle of snorts and wrinkles.', photos: [], friends: ['pet-4'], privacySettings: { profile: 'public', playdates: 'friends' } },
];
...
function createSchema() {
    db.run(`
        CREATE TABLE users (
            id TEXT PRIMARY KEY, name TEXT, username TEXT, avatarUrl TEXT, role TEXT, status TEXT, privacySettings TEXT
        );
        CREATE TABLE pets (
            id TEXT PRIMARY KEY, ownerId TEXT, name TEXT, breed TEXT, avatarUrl TEXT, type TEXT, birthdate TEXT, bio TEXT, photos TEXT, privacySettings TEXT, healthLog TEXT, achievements TEXT, favoriteItems TEXT
        );
        ...
        CREATE TABLE posts (id TEXT PRIMARY KEY, userId TEXT, petId TEXT, content TEXT, date TEXT, likeCount INTEGER);
...
function seedData() {
    allInitialUsers.forEach(user => {
        db.run("INSERT INTO users VALUES (?, ?, ?, ?, ?, ?, ?)", [user.id, user.name, user.username, user.avatarUrl, user.role, user.status, JSON.stringify(user.privacySettings)]);
    });
    allInitialPets.forEach(pet => {
        db.run("INSERT INTO pets VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [pet.id, pet.ownerId, pet.name, pet.breed, pet.avatarUrl, pet.type, pet.birthdate, pet.bio, JSON.stringify(pet.photos), JSON.stringify(pet.privacySettings), '[]', '[]', '[]']);
    ...
    initialPosts.forEach(p => db.run("INSERT INTO posts VALUES (?, ?, ?, ?, ?, ?)", [p.id, p.userId, p.petId, p.content, p.date, p.likeCount]));
...
const rowToPet = (row: any[]): Pet & {ownerId: string} => ({
    id: row[0], ownerId: row[1], name: row[2], breed: row[3], avatarUrl: row[4], type: row[5], birthdate: row[6], bio: row[7], photos: JSON.parse(row[8]), privacySettings: JSON.parse(row[9]), healthLog: JSON.parse(row[10]), achievements: JSON.parse(row[11]), favoriteItems: JSON.parse(row[12]), friends: []
});
...
export async function getAllData() {
    ...
    return {
        users,
        posts: execAndMap("SELECT * FROM posts", r => ({ id: r[0], userId: r[1], petId: r[2], content: r[3], date: r[4], likeCount: r[5], type: 'post' }) as Post),
...
// --- Mutations ---
export async function createPost(userId: string, content: string, petId?: string) {
    db.run("INSERT INTO posts VALUES (?, ?, ?, ?, ?, ?)", [`post-${Date.now()}`, userId, petId, content, new Date().toISOString(), 0]);
    await saveDbToIndexedDB();
}
...
export async function likeFeedItem(userId: string, itemId: string) {
    ...
    const isLiked = isLikedResult.length > 0 && isLikedResult[0].values.length > 0;
