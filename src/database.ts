...
// --- INITIAL SEED DATA ---
// FIX: Added 'bio' property to each user object to match the User type definition.
...
function createSchema() {
    // FIX: Added the 'bio' column to the users table to match the User type.
    db.run(`
        CREATE TABLE users (
...
function seedData() {
    // FIX: Added 'bio' to the user insertion query to match the updated schema.
...
// FIX: Added 'bio' property from row[7] to the returned user object to match the User type.
