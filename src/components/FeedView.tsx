    ...
    const visibleFeedItems: EnrichedFeedItem[] = React.useMemo(() => {
        ...
        feedItems.forEach(item => {
            if (visibleUserIds.has(item.userId)) {
                ...
                // FIX: Add a guard to ensure owner is not undefined. This helps TypeScript
                // correctly infer the type of `feed` items, as `EnrichedPost` and
                // `EnrichedActivity` require a non-nullable `user` property.
                if (owner) {
        ...
        return feed
            .filter(item => {
                switch(activeFilter) {
                    ...
                    // FIX: Changed `item.userId` to `item.user.id` to fix the type error.
                    // The `user` object is guaranteed to exist on `EnrichedFeedItem`.
